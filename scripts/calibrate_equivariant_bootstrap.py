...
(dataset, total_intensity). The estimator is always ``FBPUNet``.

Procedure
---------
For each dataset (lung, composite, lamino):
  For each intensity in [1e4, 1e5, 1e6, 1e7, 1e8, 1e9]:
    For each (rotation_std_deg, flip) in {2, 4, 8, 16, 32, 64, 128}
                                          × {False, True}:
      Run the equivariant bootstrap with n_bootstraps=100 on the first 10
      test-set images. For the single error level alpha = 0.05 (alpha =
      1 - target coverage in this codebase), build a percentile bootstrap
      interval per pixel and compute empirical coverage within the circular
      mask. Score = mean |emp_cov - (1 - alpha)|.
    Pick the (sigma, flip) with the smallest score.

The image range [0, 10) sits *before* the paper sweep [10, 110), so calibration
does not bleed into the reported test set.

Calibrated parameters land in ``uqct/settings.toml`` as a list of TOML
records under ``[[eval-sparse.equivariant_bootstrapping.calibrated]]``.
"""
