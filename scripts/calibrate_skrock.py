...
SK-ROCK's posterior width -- hence its calibration -- is governed mainly by the TV
weight: a stronger prior shrinks the credible intervals (at the cost of a little
bias). ``uqct.other_methods.skrock`` sets ``beta = tv_weight_calibration *
_calibrate_tv_weight(intensity)`` (the MAP heuristic times a calibration factor,
default ``2.0``); this script sweeps the multiplier and keeps, per cell, the value
with the smallest **Expected Calibration Error** over the nominal grid ``Q`` -- a
pure ECE objective, with no coverage floor (ties within ``ECE_TOL`` are broken
toward the *larger* beta, i.e. tighter intervals / less posterior-mean bias).
``cov@95``, ``CIw@95`` and posterior-mean PSNR are reported for context only.

Procedure (mirrors ``scripts/calibrate_equivariant_bootstrap.py``)
------------------------------------------------------------------
For each dataset, for each intensity, for each multiplier in ``MULTIPLIERS``: run
SK-ROCK on the first 10 test-set images (``image_range = (0, 10)``, which sits
before the reported sweep ``[10, 110)``), form per-pixel percentile intervals at
the nominal levels ``Q``, and record ``ECE = mean_q mean_image |emp_cov(q) - q|``
(plus cov@95 / CIw@95 / PSNR) within the circular mask. Keep the smallest-ECE
multiplier; if it differs from the production default (``2.0``) emit a per-cell
override.

Per-cell results are cached on disk (``results/caches/skrock_calib_cache.json``,
keyed by dataset / intensity / multiplier / sampler budget) so re-runs -- e.g. to
add more multipliers -- only compute the new (cell, multiplier) combinations.

Per-cell overrides land in ``uqct/settings.toml`` under
``[[eval-sparse.skrock.calibrated]]`` (consumed by ``uqct.eval.cli``; the
standalone ``uqct.eval.skrock`` uses ``tv_weight_calibration`` directly). A full
sweep over ``DATASETS x INTENSITIES`` rewrites the whole block; a partial sweep
merges with the cells it did not touch.

This is compute-heavy (one SK-ROCK run per uncached cell); the iteration counts
below are deliberately smaller than the production defaults -- enough for the
posterior *width* (and roughly the mean) to settle, which is all calibration needs.
"""
...
def main() -> None:
    ...
    covered = {(d, float(i)) for d in DATASETS for i in INTENSITIES}
