...
def compute_nll_trajectory(
    ...
    if S != S_sched:
         raise ValueError(f"images schedule dim {S} does not match schedule length {S_sched}")
    ...
    split_points = torch.cat([schedule.to(device), torch.tensor([len(experiment.angles)], device=device)])
    start = schedule.min()
...
def compute_constraint_violation(
    nll_cum: torch.Tensor,
    threshold_trajectory_cum: torch.Tensor
) -> torch.Tensor:
    """
