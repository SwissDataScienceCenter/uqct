...
def load_runs(
    ...
    if runs_dir.is_file():
        click.echo(f"Loading consolidated file: {runs_dir}")
        try:
            df = pd.read_parquet(runs_dir)
            if "model" not in df.columns:
                 # Should not happen if created by consolidate_runs
                 click.echo("Error: Consolidated file missing 'model' column.")
                 return {}
            ...
            click.echo(f"Error loading consolidated file: {e}")
            return {}
    ...
    click.echo(f"Found {len(files)} files in {runs_dir}")
        ...
        try:
            ...
            if len(parts) >= 6:
                ...
                if dataset and parts[1] != dataset:
                    continue
                if intensity is not None and not math.isclose(float(parts[2]), intensity):
                    continue
            ...
            if df.empty:
                click.echo(f"Skipping {file_path.name}: empty dataframe")
                continue
            ...
            if dataset and row_ds != dataset:
                click.echo(f"Skipping {file_path.name}: dataset mismatch {row_ds} != {dataset}")
                continue
            if intensity is not None and not math.isclose(row_int, intensity):
                click.echo(f"Skipping {file_path.name}: intensity mismatch {row_int} != {intensity}")
                continue
            if sparse is not None and row_sp != sparse:
                click.echo(f"Skipping {file_path.name}: sparse mismatch {row_sp} != {sparse}")
                continue
            ...
            if start == -1:
                 click.echo(f"Warning: Could not determine image range for {file_path}")
                 continue
            ...
            # click.echo("Appended candidate")

        except Exception as e:
            # click.echo(f"Error processing {file_path}: {e}")
            continue

    # click.echo(f"Total candidates accepted: {len(candidates)}")
    ...
    for cfg, file_list in config_candidates.items():
        ...
        # click.echo(f"Aggregated {cfg}: {len(full_df)} images")
