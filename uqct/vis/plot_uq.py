...
def _metrics_from_precomputed(
    ...
    ret: dict[str, float] = {}
    metric_cols = [
        c for c in sub.columns if c.endswith(("_width", "_ind_cov", "_sim_cov"))
    ]
...
def main(n_bootstraps, cache, out):
    ...
    if todo:
        ...
        for dataset, method in tqdm(todo, desc="Missing cells", leave=True):
            ...
            for intensity in tqdm(
                TOTAL_INTENSITIES, desc=f"{dataset}/{method}", leave=False
            ):
    ...
    serializable = {
        d: {
            m: dict(results[d][m])
            if not isinstance(results[d][m], dict)
            else results[d][m]
            for m in results[d]
        }
        for d in results
    ...
    for metric_key in metrics_to_plot:
        ...
        for row_idx, dataset in enumerate(DATASETS):
            ...
            for method in METHODS:
                ...
                band_lo = vals_plot - std_plot
                band_hi = vals_plot + std_plot
                if "_cov" in metric_key.lower():
                    band_lo = np.clip(band_lo, 0.0, 100.0)
                    band_hi = np.clip(band_hi, 0.0, 100.0)
                ax.fill_between(
                    ints[sort_idx],
                    band_lo,
                    band_hi,
        ...
        for r_idx, metric_key in enumerate(combined_metrics):
            ...
            for c_idx, dataset in enumerate(DATASETS):
                ...
                for method in METHODS:
                    ...
                    # Coverage is bounded in [0, 100]; clip the ±std band so it
                    # doesn't spill past the physical range.
                    band_lo = vals_plot - std_plot
                    band_hi = vals_plot + std_plot
                    if "_cov" in metric_key:
                        band_lo = np.clip(band_lo, 0.0, 100.0)
                ...
                ax.set_xscale("log")
                # Only major horizontal grid lines (no vertical / minor lines).
                ax.grid(True, which="major", axis="y", linestyle="--", alpha=0.3)
