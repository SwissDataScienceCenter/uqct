...
def compute_rotated_nll(gt: torch.Tensor, angles_deg: List[float], experiment, schedule) -> Dict[float, List[float]]:
    """
    ...
    for angle in angles_deg:
        try:
             rot_np = ndimage.rotate(gt_np, angle, axes=(1, 2), reshape=False, order=1, mode='constant', cval=0.0, prefilter=False)
        except TypeError:
             rot_list = []
             for i in range(n_images):
                  rot_list.append(ndimage.rotate(gt_np[i], angle, reshape=False, order=1, mode='constant', cval=0.0, prefilter=False))
...
@click.option(
    ...
    default=Path("./plots/rotation"),
    help="Directory to save plots.",
)
@click.option("--limit", default=None, type=int, help="Limit number of groups processing (debug).")
...
def main(consolidated_file: Path, output_dir: Path, device: str, limit: Optional[int]):
    ...
    try:
        ...
        click.echo(f"Could not load settings.toml ({e}), defaulting to {default_schedule_len}")
    ...
    if missing:
        click.echo(f"Missing columns for grouping: {missing}. Cannot proceed with analysis.")
        return
    ...
    for group_keys, group_df in tqdm(groups, total=len(groups)):
        ...
        if (t1 - t0) > 0.5 or (t2 - t1) > 0.5:
                click.echo(f"  [Group {count_groups}] Setup: {t1-t0:.2f}s | Calc: {t2-t1:.2f}s")
            ...
            for i in range(n_images):
                ...
                for angle in angles_deg:
                    ...
                    if len(rot_cum) != len(thresh_cum):
                            click.echo(f"Warning: Length Mismatch Model {model}: Rot {len(rot_cum)} vs Thresh {len(thresh_cum)}")
                            continue
    ...
    agg = res_df.groupby(["dataset", "intensity", "sparse", "model", "angle"])["excluded"].mean().reset_index()
    ...
    for (d, inten, sp), p_df in plot_groups:
        ...
        models = p_df["model"].unique()
        for m in models:
            sub = p_df[p_df["model"] == m].sort_values("angle")
            plt.plot(sub["angle"], sub["exclusion_rate"], label=m, marker='o', alpha=0.8)
            
        plt.xscale("log")
        plt.xlabel("Rotation Angle (Degrees) - Log Scale")
        ...
        plt.savefig(target_dir / "exclusion_rate.png")
        plt.close()
    ...
    global_agg = agg.groupby(["intensity", "model", "angle"])["exclusion_rate"].mean().reset_index()
    ...
    if len(intensities) > 0:
        ...
        fig_stack, axes_stack = plt.subplots(len(intensities), 1, figsize=(10, 4 * len(intensities)), sharex=True)
        ...
        for idx, inten in enumerate(intensities):
            ...
            # Individual Plot
            plt.figure(figsize=(8, 6))
            models = sorted(p_df["model"].unique())
            for m in models:
                sub = p_df[p_df["model"] == m].sort_values("angle")
                plt.plot(sub["angle"], sub["exclusion_rate"], label=m, marker='o', alpha=0.8)
                
                # Add to Stacked
                axes_stack[idx].plot(sub["angle"], sub["exclusion_rate"], label=m, marker='o', alpha=0.8)
            ...
            plt.savefig(global_dir / f"exclusion_global_{inten_str}.png")
            plt.close()
            ...
            axes_stack[idx].legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small')

        # Finalize Stacked
        axes_stack[-1].set_xlabel("Rotation Angle (Degrees) - Log Scale")
        axes_stack[-1].set_xscale("log")
        
        plt.tight_layout()
        fig_stack.savefig(global_dir / "exclusion_global_stacked.png")
        plt.close(fig_stack)
