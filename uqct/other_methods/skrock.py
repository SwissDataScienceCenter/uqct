"""SK-ROCK posterior sampler for the Beer-Lambert/Poisson CT model.

Built on top of ``deepinv.sampling.SKRockIterator`` with a log-Poisson data
fidelity (``BeerLambertLikelihood``, equivalent to ``uqct.ct.nll`` up to an
``x``-independent constant) and either a Moreau-Yosida TV prior or no prior.

The TV weight ``beta`` is auto-calibrated from the total intensity by the
codebase's MAP heuristic. The calibration multiplier ``tv_weight_calibration``
(default ``2.0``) is a global fallback; per-(dataset, intensity) ECE-optimal
overrides from ``scripts/calibrate_skrock.py`` are applied via the eval CLI.
Pass ``tv_weight`` to override directly.

SK-ROCK only uses first-order gradients, so no double-backward through ASTRA
is required. Output shape: ``(N, T=1, R, H, W)`` with ``R = n_samples``
