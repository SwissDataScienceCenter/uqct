"""Equivariant bootstrap of Tachella & Pereyra (AISTATS 2024) with the
``FBPUNet`` estimator as ``x_hat``.

For i = 1 ... N: (1) draw a group element ``g_i`` (in-plane rotation
``theta_i ~ N(0, sigma^2)``, optionally with axis flips); (2) simulate
``y_tilde_i ~ P(A T_{g_i} x_hat)``; (3) reconstruct ``x_hat_i =
FBPUNet(y_tilde_i)``; (4) invert: ``x_tilde_i = T_{g_i}^-1 x_hat_i``.
The empirical ``{x_tilde_i}`` approximates the sampling distribution of
``FBPUNet(Y)``.
"""
